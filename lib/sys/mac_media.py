...
def setup_mac_media_controller(state: "PlayerState") -> None:
    ...
    def update_now_playing_info() -> None:
        ...
        now_playing_info[MPMediaItemPropertyPlaybackDuration] = NSNumber.numberWithDouble_(total_time_s)
        
        current_time_s: float = state.stream.current_time.value / 1e9
        now_playing_info[MPNowPlayingInfoPropertyElapsedPlaybackTime] = NSNumber.numberWithDouble_(current_time_s)
        
        play_rate: float = 1.0 if state.state.value == PlayState.PLAYING else 0.0
        now_playing_info[MPNowPlayingInfoPropertyPlaybackRate] = NSNumber.numberWithDouble_(play_rate)
